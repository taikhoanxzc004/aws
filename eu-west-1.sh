...
aws ec2 run-instances --image-id ami-00f117fe174f83c56 --key-name KeyPair --security-groups Group --instance-type c7g.medium --count 5 --block-device-mappings 'DeviceName=/dev/sda1,Ebs={VolumeSize=100}' --user-data fileb:///root/AWS-ARM-internetincome.sh --region eu-west-1
