...
aws ec2 run-instances --image-id ami-083644ad45483a44f --key-name KeyPair --security-groups Group --instance-type c7g.medium --count 5 --block-device-mappings 'DeviceName=/dev/sda1,Ebs={VolumeSize=100}' --user-data file:///root/AWS-ARM-internetincome.sh" --region ap-southeast-2
